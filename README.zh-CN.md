...
| 能力 | 说明 |
| --- | --- |
| **Self-evolving by design** | 通过 `soul_update`、`memory_write`、`skill_create`、tools 和 `upgrade_code`，让 Agent 不停留在静态能力集 |
...
- `memory_write` 不是一个“合并层”，而是统一入口: `target=user` 写入 `USER` 层，`target=memory` 写入 `MEMORY` 层
- `tool_list` 用 `layers` 字段暴露工具所属层级；同一个工具可以对应多个层，例如 `memory_write` 会返回 `["user", "memory"]`
...
- `USER`: `memory_write(target=user)` + `USER.md`
- `MEMORY`: `memory_write(target=memory)` + `MEMORY.md`
- `SKILL`: `skill_create` / `skill_list` + `skills/`
- `TOOL`: `tool_create` / `tool_list` / `tool_delete` + `tools/`
- `CODE`: `reflect` / `upgrade_code` / `CodeUpgrade` / `UpgradeManager`

这里有一个容易混淆的点:

- `USER` 和 `MEMORY` 在概念上是两层，不应合并
- `memory_write` 只是同时服务这两层的统一写入口
- 因此 `tool_list` 会把它表示为多层归属，而不是把两层压成一个单独标签
