...
12. app artifact 改动后在 Workbench 里点当前 app 的 `Reload` 生效；只有 Workbench core CODE 改动才需要 `self_update deploy`。
...
<head>
  <meta charset="utf-8">
  <link rel="stylesheet" href="/app-assets/hello/style.css">
</head>
<body>
  <main id="app"></main>
  <script src="/app-assets/hello/app.js"></script>
</body>
</html>
```
...
- search/backlink 只有在 file API 稳定后再做 read-only view
...
- `notes:write` 用于 save notes。
- 文件路径必须 workspace-relative，并限制在明确 notes root 内。
- 后端文件访问要复用现有 workspace/security helpers。
...
- 不重新引入 `runtime.kind` / Vite / build / HMR 作为 app manifest 要求。
