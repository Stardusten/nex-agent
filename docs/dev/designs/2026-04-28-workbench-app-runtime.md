...
Workbench shell 第一版可以是 no-build static HTML。核心 runtime 不要求 Vite、React、HMR 或 Node dev server。

Workbench app 采用标准静态 artifact contract：
...
复杂 app 可以自由使用 React、TypeScript、CodeMirror、Vite 或其他前端栈，但这些都是 app-local source/build 选择，不进入 Workbench core runtime contract。复杂 app 用可选 `reload.sh` 把 source materialize 成 manifest `entry` 指向的静态产物。
...
`reload.sh` 是 app-local artifact refresh hook，不是 core CODE deploy，也不是 browser iframe reload。
...
`Nex.Agent.Runtime.Config.workbench_runtime/1` 负责规范化，`Nex.Agent.Runtime.Snapshot.workbench.runtime` 负责投影给长期进程。Workbench server 只能从 Runtime snapshot 读端口，不直接读取 config 文件。
...
- notes app 自身是 workspace app artifact。
- 笔记库真相源可以是 NexAgent workspace 外的 Obsidian vault；它不是第二个 NexAgent workspace。
- app 通过 `root_id` + vault-relative path 调用 bounded notes bridge，不传 absolute path。
- 后端把 `root_id` 解析到显式配置的 notes root，并复用 file-access allowed roots / `Nex.Agent.Sandbox.Security`。
...
`reload.sh` 是可选的 app-local 构建/准备脚本。后续受控 runner 只能按 app id 解析到该 app 目录、以该目录为 cwd 执行该脚本、限制 timeout/output、写 ControlPlane observations，并返回 bounded 结果。iframe app 不得通过 SDK bridge 直接执行 `reload.sh`。
...
workbench.app.reload.started
workbench.app.reload.finished
workbench.app.reload.failed
...
4. agent 生成或修改 `nex.app.json`、前端源码、必要 package 配置和可选 `reload.sh`。
5. simple app 直接刷新 iframe；buildable app 先由受控 app reload/build lane 运行 `reload.sh`。
6. Workbench build/check 通过后生成或更新 candidate。
7. owner 批准权限和 app 安装。
8. shell 加载新 app。
9. 所有生命周期写入 ControlPlane。
...
- `TOOL`：行情查询、文件读写、受控 app reload/build、权限审批等确定性能力。
...
1. `reload.sh` 的受控 runner 是由 Elixir 同步执行，还是进入后台 task/supervisor？
