...
- `Nex.Agent.Extension.Plugin.Template` provides the common `{{...}}` plugin config/workspace/session/turn rendering boundary; plugin service credentials are ordinary config fields and are rendered only where the caller places them.
...
- Stage 6 fake external memory acceptance proves recall hook injection, retain debounce merge, streamable HTTP MCP calls, direct config header rendering, empty header omission, bank template rendering, disabled plugin behavior, and permission deny.
