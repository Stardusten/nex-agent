...
3. `docs/dev/findings/*.md`
4. `docs/dev/task-plan/*.md`
5. `docs/dev/designs/*.md`
6. 本文档
...
- `docs/dev/designs/`：设计草稿、方案推演、开放问题
...
- 设计草稿、方案比较、开放问题：放 `docs/dev/designs/`
- 架构结论、最终取舍、调查结论：放 `docs/dev/findings/`
...
不要把 phase 文档写成动机散文。
还在比较方案时先写 `docs/dev/designs/`；等选择收敛后再把稳定结论沉淀成 finding，并从 finding/task-plan 进入实现。
