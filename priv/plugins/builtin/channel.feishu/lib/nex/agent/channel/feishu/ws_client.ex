defmodule Nex.Agent.Channel.Feishu.WSClient do
  ...
  def start_link(url, headers, parent_pid) do
    ...
    Nex.Agent.Interface.WS.start_link(url, __MODULE__, state, extra_headers: headers)
  end
  ...
  def send_frame(pid, %Frame{} = frame) when is_pid(pid) do
    Nex.Agent.Interface.WS.cast(pid, {:send_frame, frame})
  end
  ...
  def handle_connect(_conn, state) do
    ...
    Nex.Agent.Interface.WS.cast(self(), :send_initial_ping)
    {:ok, state}
  end
  ...
  defp handle_control_frame(frame, state) do
    ...
    case type do
      "ping" ->
        ...
        Nex.Agent.Interface.WS.cast(self(), {:send_frame, pong})
        state
