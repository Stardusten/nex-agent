defmodule Nex.Agent.Observe.ControlPlane.Metric do
  @moduledoc false
  ...
  defmacro count(name, value, attrs, opts \\ []) do
    ...
    quote bind_quoted: [name: name, value: value, attrs: attrs, opts: opts, source: source] do
      Nex.Agent.Observe.ControlPlane.Metric.emit("count", name, value, attrs, opts, source)
    end
  end
  ...
  defmacro measure(name, value, attrs, opts \\ []) do
    ...
    quote bind_quoted: [name: name, value: value, attrs: attrs, opts: opts, source: source] do
      Nex.Agent.Observe.ControlPlane.Metric.emit("measure", name, value, attrs, opts, source)
    end
  end
