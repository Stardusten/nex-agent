defmodule Nex.Agent.Observe.Admin do
  @moduledoc false
  ...
  alias Nex.Agent.Capability.Tool.{CustomTools, Registry}
  alias Nex.Agent.Interface.Workbench.Store, as: WorkbenchStore
  require Log

  @event_topic :admin_events
  @max_preview_lines 120
  @personal_task_plugin_id "builtin:workflow.personal-task"
  ...
  def rollback_code_update(module_name, version_id, opts) when is_binary(module_name) do
    with {:ok, _module, _path} <- resolve_code_deploy_target(module_name) do
      result = Update.Deployer.rollback(if(present?(version_id), do: version_id, else: nil))
  ...
  defp tasks_summary(opts) do
    summary =
      if personal_task_plugin_enabled?(opts) do
        personal_tasks_summary(opts)
      else
        empty_personal_tasks_summary()
      end
  ...
  defp personal_tasks_summary(opts) do
    case execute_tool("task", %{"action" => "summary", "scope" => "all"}, tool_ctx(opts)) do
      {:ok, summary} when is_map(summary) -> summary
      _ -> empty_personal_tasks_summary()
    end
  end

  defp empty_personal_tasks_summary do
    %{
      "open" => 0,
      "completed" => 0,
      "upcoming" => [],
  ...
  defp personal_tasks_list(opts) do
    if personal_task_plugin_enabled?(opts) do
      case execute_tool("task", %{"action" => "list"}, tool_ctx(opts)) do
        {:ok, %{"tasks" => tasks}} when is_list(tasks) -> tasks
        {:ok, %{tasks: tasks}} when is_list(tasks) -> tasks
        _ -> []
      end
    else
  ...
  defp releases_for(module_name) do
    Nex.Agent.Self.Update.ReleaseStore.list_releases()
  ...
  defp tool_ctx(opts) do
    ...
    |> maybe_put_keyword(:runtime_snapshot, admin_runtime_snapshot(opts))
    |> Enum.into(%{})
  end
  ...
  defp personal_task_plugin_enabled?(opts) do
    case admin_runtime_snapshot(opts) do
      %Nex.Agent.Runtime.Snapshot{workspace: snapshot_workspace, plugins: plugins}
      when is_binary(snapshot_workspace) ->
        if same_workspace?(snapshot_workspace, workspace(opts)) do
          plugin_enabled?(plugins, @personal_task_plugin_id)
        else
          true
        end

      %Nex.Agent.Runtime.Snapshot{plugins: plugins} ->
        plugin_enabled?(plugins, @personal_task_plugin_id)
  ...
  defp admin_runtime_snapshot(opts) do
  ...
  defp plugin_enabled?(%{enabled: enabled}, plugin_id) when is_list(enabled),
    do: plugin_id in enabled

  defp plugin_enabled?(%{"enabled" => enabled}, plugin_id) when is_list(enabled),
    do: plugin_id in enabled

  defp plugin_enabled?(_plugins, _plugin_id), do: true
