defmodule Nex.Agent.Workbench.ConfigPanel do
  @moduledoc false
  ...
  def upsert_provider(provider_key, attrs, %Snapshot{} = snapshot) when is_map(attrs) do
    update_config(snapshot, "provider", provider_key, fn raw ->
      with {:ok, key} <- normalize_key(provider_key, "provider"),
           ...
           {:ok, type} <- provider_type(Map.get(attrs, "type") || Map.get(existing, "type")),
  ...
  def upsert_channel(channel_id, attrs, %Snapshot{} = snapshot) when is_map(attrs) do
    update_config(snapshot, "channel", channel_id, fn raw ->
      with {:ok, key} <- normalize_key(channel_id, "channel"),
           ...
           {:ok, type} <- channel_type(Map.get(attrs, "type") || Map.get(existing, "type")),
           {:ok, entry} <- channel_entry(type, existing, attrs) do
  ...
  defp config_view(raw, %Snapshot{} = snapshot) do
    ...
    %{
      ...
      "channels" => channel_views(raw, config)
    }
  end
  ...
  defp channel_views(raw, %Config{} = config) do
    raw
    |> channel_entries()
    |> Enum.sort_by(fn {key, _entry} -> key end)
    |> Enum.map(fn {key, entry} ->
  ...
  defp provider_type(nil), do: {:error, "provider type is required"}
  defp provider_type(type) when type in @provider_types, do: {:ok, type}
