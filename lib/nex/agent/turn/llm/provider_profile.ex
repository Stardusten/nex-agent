defmodule Nex.Agent.Turn.LLM.ProviderProfile do
  @moduledoc false
  ...
  @spec stream_text_fun(t()) :: Nex.Agent.Turn.LLM.ProviderAdapter.stream_text_fun()
  ...
  @spec forced_tool_choice(t() | atom(), String.t(), keyword()) :: map() | nil
  def forced_tool_choice(provider_or_profile, name, opts \\ [])

  def forced_tool_choice(%__MODULE__{} = profile, name, _opts) when is_binary(name) do
    profile
    |> adapter_for_profile()
    |> call_adapter(:forced_tool_choice, [profile, name])
  end

  def forced_tool_choice(provider, name, opts) when is_atom(provider) and is_binary(name) do
    provider
    |> __MODULE__.for(opts)
    |> forced_tool_choice(name)
  end
  ...
  defp call_adapter(adapter, function, args) do
    ...
    if function_exported?(adapter, function, length(args)) do
      ...
      apply(Nex.Agent.Turn.LLM.Providers.Default, function, args)
    end
  end
