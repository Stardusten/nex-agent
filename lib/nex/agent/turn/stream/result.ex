defmodule Nex.Agent.Turn.Stream.Result do
  @moduledoc false
...
defimpl String.Chars, for: Nex.Agent.Turn.Stream.Result do
  def to_string(%Nex.Agent.Turn.Stream.Result{final_content: content}) when is_binary(content),
    do: content

  def to_string(%Nex.Agent.Turn.Stream.Result{error: error, status: :error}), do: inspect(error)
