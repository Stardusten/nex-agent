defmodule Nex.Agent.Turn.Runner do
  @moduledoc false
  ...
  defp do_run(session, prompt, opts) do
    ...
    default_model_runtime =
      Keyword.get(opts, :model_runtime) ||
        if(runtime_config, do: Nex.Agent.Runtime.Config.default_model_runtime(runtime_config))
    ...
    model =
      Keyword.get(opts, :model) ||
        if(default_model_runtime,
          ...
          else: Nex.Agent.Turn.LLM.ProviderProfile.default_model(provider)
        )
  ...
  defp maybe_publish_tool_results(results, opts) do
    if Process.whereis(Nex.Agent.App.Bus) do
  ...
  defp default_model_runtime(%Snapshot{config: %Nex.Agent.Runtime.Config{} = config}) do
    Nex.Agent.Runtime.Config.default_model_runtime(config)
  end
  ...
  defp call_req_llm_stream(messages, opts, callback) do
    ...
    Nex.Agent.Turn.LLM.ReqLLM.stream(messages, stream_opts, callback)
  end
  ...
  defp parse_args(args) when is_binary(args) do
    case Jason.decode(args) do
      ...
      _ ->
        case Nex.Agent.Turn.LLM.JsonRepair.repair_and_decode(args) do
          {:ok, map} -> map
          _ -> %{}
        end
    end
  ...
  defp tool_registry_resolution_opts(opts, filter) do
    ...
    |> maybe_put_opt(:plugin_data, runtime_plugins(runtime_snapshot))
    |> maybe_put_opt(:model_runtime, default_model_runtime(runtime_snapshot))
    |> maybe_put_opt(:surface, filter)
  end

  defp runtime_plugins(%Snapshot{plugins: plugins}) do
    if plugin_projection_empty?(plugins), do: nil, else: plugins
  end

  defp runtime_plugins(_snapshot), do: nil
  ...
  defp call_llm_stream_for_consolidation(messages, opts) do
    stream_fun = fn callback ->
      Nex.Agent.Turn.LLM.ReqLLM.stream(messages, opts, callback)
    end
