defmodule Nex.Agent.Interface.Gateway do
  @moduledoc """
  ...
  @impl true
  def init(_opts) do
    config = runtime_config() || Nex.Agent.Runtime.Config.load()
  ...
  def handle_call(:status, _from, state) do
    model_runtime = Nex.Agent.Runtime.Config.default_model_runtime(state.config)
    ...
    status = %{
      ...
      services: %{
        bus: Process.whereis(Nex.Agent.App.Bus) != nil,
        cron: Process.whereis(Nex.Agent.Capability.Cron) != nil,
        heartbeat: Process.whereis(Nex.Agent.App.Heartbeat) != nil,
        tool_registry: Process.whereis(Nex.Agent.Capability.Tool.Registry) != nil,
        inbound_worker: Process.whereis(Nex.Agent.Conversation.InboundWorker) != nil,
        subagent: Process.whereis(Nex.Agent.Capability.Subagent) != nil,
        channels:
          state.config
          |> Nex.Agent.Runtime.Config.channel_instances()
          |> Enum.into(%{}, fn {id, instance} ->
            {id,
             %{
               type: Map.get(instance, "type"),
               enabled: Map.get(instance, "enabled", false) == true,
               running: Nex.Agent.Interface.Channel.Registry.whereis(id) != nil
             }}
          end)
      }
    }
  ...
  defp do_start(state) do
    ...
    if Nex.Agent.Runtime.Config.valid?(config) do
      # Start heartbeat tick (may not be running if started outside OTP app)
      if Process.whereis(Nex.Agent.App.Heartbeat) do
        _ = Nex.Agent.App.Heartbeat.start()
      end
  ...
  defp do_stop(state) do
    # Stop heartbeat tick
    if Process.whereis(Nex.Agent.App.Heartbeat) do
      _ = Nex.Agent.App.Heartbeat.stop()
    end
  ...
  defp reconcile_channels(old_config, new_config) do
    old_instances = Nex.Agent.Runtime.Config.enabled_channel_instances(old_config)
    new_instances = Nex.Agent.Runtime.Config.enabled_channel_instances(new_config)
  ...
  defp channel_specs(config) do
    config
    |> Nex.Agent.Runtime.Config.enabled_channel_instances()
  ...
  defp do_send_message(message) do
    ...
    config = if snapshot, do: snapshot.config, else: Nex.Agent.Runtime.Config.load()
    model_runtime = Nex.Agent.Runtime.Config.default_model_runtime(config)
