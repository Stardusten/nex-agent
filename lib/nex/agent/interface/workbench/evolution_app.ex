defmodule Nex.Agent.Interface.Workbench.EvolutionApp do
  @moduledoc false
  ...
  defp do_perform_action(candidate_id, action, args, opts) do
    with :ok <- ensure_confirmed(args),
      ...
      case execute_tool("evolution_candidate", tool_args, tool_ctx(opts)) do
        {:ok, result} -> {:ok, result}
  ...
  defp tool_ctx(opts) do
    ...
    case Runtime.current() do
      {:ok, snapshot} -> Map.put(ctx, :runtime_snapshot, snapshot)
      _ -> ctx
    end
  end
