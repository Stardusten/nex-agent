defmodule Nex.Agent.Interface.Workbench.Router do
  @moduledoc false

  alias Nex.Agent.Observe.ControlPlane.Query
  alias Nex.Agent.Runtime.Snapshot

  alias Nex.Agent.Interface.Workbench.{
    AppManifest,
    Assets,
    Bridge,
    ConfigPanel,
  ...
  def dispatch(method, target, body, %Snapshot{} = snapshot) do
    ...
    case {method, segments} do
      ...
      {"GET", ["api", "workbench", "skills"]} ->
        {200, SkillsApp.overview(snapshot, query_params)}

      {"GET", ["api", "workbench", "skills", skill_id]} ->
        case SkillsApp.detail(skill_id, snapshot) do
          {:ok, view} -> {200, view}
          {:error, reason} -> {404, %{"error" => reason}}
        end
      ...
      {"POST", ["api", "workbench", "skills"]} ->
        with {:ok, args} <- decode_json_object(body),
             {:ok, view} <- SkillsApp.save(args, snapshot) do
          {200, view}
        else
          {:error, reason} -> {400, %{"error" => reason}}
        end
      ...
      {"PUT", ["api", "workbench", "skills", skill_id]} ->
        with {:ok, args} <- decode_json_object(body),
             {:ok, view} <-
               SkillsApp.save(Map.put(args, "name", workspace_name(skill_id)), snapshot) do
          {200, view}
        else
          {:error, reason} -> {400, %{"error" => reason}}
        end

      {"DELETE", ["api", "workbench", "skills", skill_id]} ->
        case SkillsApp.delete(skill_id, snapshot) do
          {:ok, view} -> {200, view}
          {:error, reason} -> {400, %{"error" => reason}}
        end

      {"POST", ["api", "workbench", "skills", skill_id, "publish"]} ->
        case SkillsApp.publish(skill_id, snapshot) do
          {:ok, view} -> {200, view}
          {:error, reason} -> {400, %{"error" => reason}}
        end
  ...
  defp scheduled_error_status("not_found", reason), do: {404, %{"error" => reason}}
  defp scheduled_error_status("unavailable", reason), do: {503, %{"error" => reason}}
  defp scheduled_error_status(_code, reason), do: {400, %{"error" => reason}}
  ...
  defp workspace_name(skill_id) do
