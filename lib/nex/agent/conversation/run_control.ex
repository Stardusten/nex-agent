defmodule Nex.Agent.Conversation.RunControl do
  @moduledoc """
  ...
  defp observe_run(tag, %Run{} = run, attrs) do
    ...
    _ =
      case tag do
        "run.owner.failed" -> Nex.Agent.Observe.ControlPlane.Log.error(tag, attrs, opts)
        "run.owner.cancelled" -> Nex.Agent.Observe.ControlPlane.Log.warning(tag, attrs, opts)
  ...
  defp ms_to_iso8601(ms) when is_integer(ms) do
    ...
    _e -> Nex.Agent.Observe.ControlPlane.Store.timestamp()
  end
