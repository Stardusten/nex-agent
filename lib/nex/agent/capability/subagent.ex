defmodule Nex.Agent.Capability.Subagent do
  @moduledoc """
  ...
  alias Nex.Agent.{App.Bus, Runtime.Config, Conversation.Session, Conversation.SessionManager}
  ...
  * `:project` - Parent project context
  ...
  defp run_subagent_loop(server, task_id, task_description, label, opts) do
    ...
    model =
      model_runtime_value(model_runtime, :model_id) ||
        Keyword.get(opts, :model, Nex.Agent.Turn.LLM.ProviderProfile.default_model(provider))
    ...
    subagent_module = Keyword.get(opts, :subagent_module, Nex.Agent.Capability.Subagent)
    ...
    try do
      case Nex.Agent.Turn.Runner.run(session, prompt, runner_opts) do
  ...
  defp render_runner_result(%Nex.Agent.Turn.Stream.Result{} = result), do: to_string(result)
