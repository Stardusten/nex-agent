defmodule Nex.Agent.Capability.Skills do
  @moduledoc """
  Markdown-only skills system.

  ## Usage

      :ok = Nex.Agent.Capability.Skills.load()
      skills = Nex.Agent.Capability.Skills.list()
      {:ok, result} = Nex.Agent.Capability.Skills.execute("explain-code", "some arguments")

      {:ok, skill} = Nex.Agent.Capability.Skills.create(%{
        name: "todo_add",
  ...
  alias Nex.Agent.{
    Capability.Skills.Catalog,
    Capability.Skills.Frontmatter,
    Capability.Skills.Loader,
    Runtime.Workspace
  }
  ...
  def create(attrs, opts \\ []) do
    ...
    disable_model_invocation =
      truthy_attr(attrs, "disable-model-invocation", :disable_model_invocation)

    always = truthy_attr(attrs, "always", :always)
  ...
  def delete(name, opts \\ []) do
    with :ok <- validate_skill_name(name) do
      ...
      if File.exists?(skill_dir) do
        ...
        if Process.whereis(@name) != nil and not Keyword.has_key?(opts, :workspace) do
  ...
  @spec all_catalog(keyword()) :: [map()]
  def all_catalog(opts \\ []), do: Catalog.all_cards(opts)
  ...
  defp save_markdown_skill(
    ...
    with :ok <- validate_skill_name(name) do
      ...
      frontmatter_lines =
        ...
        |> maybe_put_frontmatter("disable-model-invocation", disable_model_invocation)
        |> maybe_put_frontmatter("always", always)
        |> Kernel.++(["---", "", content])
  ...
  defp maybe_put_frontmatter(lines, _key, value) when value in [%{}, [], nil, false],
    do: lines

  defp maybe_put_frontmatter(lines, key, value) when is_map(value) or is_list(value) do
    lines ++ ["#{key}:"] ++ to_yaml_lines(value)
  end

  defp maybe_put_frontmatter(lines, key, value), do: lines ++ ["#{key}: #{yaml_scalar(value)}"]
  ...
  defp truthy_attr(attrs, dashed_key, atom_key) when is_map(attrs) do
    value =
      Map.get(attrs, dashed_key) ||
        Map.get(attrs, String.replace(dashed_key, "-", "_")) ||
