defmodule Nex.Agent.Self.Evolution.Candidates do
  @moduledoc false
  ...
  defp build_candidate(candidate_id, lifecycle) do
    ...
    latest = latest_lifecycle_entry(lifecycle)
  ...
  defp latest_status(lifecycle) do
    lifecycle
    |> latest_lifecycle_entry()
    |> case do
      nil -> "pending"
      entry -> Map.get(@status_by_tag, entry.tag, "pending")
    end
  end

  defp latest_lifecycle_entry(lifecycle) do
    Enum.max_by(
      lifecycle,
      fn entry ->
        status = Map.get(@status_by_tag, entry.tag, "pending")
        {Map.get(@status_rank, status, 0), entry.timestamp || "", entry.observation_id || ""}
      end,
      fn -> nil end
    )
  end
